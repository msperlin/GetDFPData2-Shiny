...
shinyServer(function(input, output, session) {
  ...
  output$user_companies <- renderText(paste0(length(input$user_companies),'
                                               companies selected'))
  output$user_companies_itr <- renderText(paste0(length(input$user_companies_itr),'
                                               companies selected'))
  ...
  #observeEvent(input$format_output,{
  ...
  output$downloadDfInfo_csv <- downloadHandler(
    ...
    content = function(con) {
      #write.csv(x = df.info.companies, file = 'DataOut/df_info.csv')
